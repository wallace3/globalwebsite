                    ...
                    <div class="lg:max-w-[635px] w-full">
                        ...
                        <div class="py-4 sm:py-6 border-b border-bdr-clr dark:border-bdr-clr-drk" data-aos="fade-up" data-aos-delay="200">
                            <div class="flex gap-4 mt-4 sm:mt-6">
                                <router-link to="/cart" class="btn btn-solid" data-text="Agregar al carrito">
...
/*computed*/
   ...
   // import LayoutOne from '@/components/product/layout-one.vue';
    ...
    const getProduct = async () => {
        try{
            const result = await fetch(`http://localhost:8080/product/${route.params.id}`)
            if(!result.ok){
    ...
    const getImages = async () => {
        try{
            const result = await fetch(`http://localhost:8080/images/${route.params.id}`)
            if(!result.ok){
                throw new Error(`Error HTTP: ${result.status}`);
            }
            const data = await result.json();
